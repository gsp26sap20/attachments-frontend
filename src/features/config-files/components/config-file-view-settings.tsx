...
import { CONFIG_FILE_LIST_FIELDS } from '@/features/config-files/view-config';
import type { ConfigFileListFieldId, ConfigFileListFieldOption } from '@/features/config-files/view-config';

function matchesSearch(field: ConfigFileListFieldOption, search: string) {
...
interface ConfigFileViewSettingsProps {
  onSelectChange?: (selectedIds: ConfigFileListFieldId[]) => void;
}

export function ConfigFileViewSettings({ onSelectChange }: ConfigFileViewSettingsProps) {
  const selectedIds = useViewStore((state) => state.configFileListVisibleFieldIds);
  const setSelectedIds = useViewStore((state) => state.setConfigFileListVisibleFieldIds);
  const [open, setOpen] = React.useState(false);
  const [search, setSearch] = React.useState('');
  const [showSelectedOnly, setShowSelectedOnly] = React.useState(false);
  const [draftSelectedIds, setDraftSelectedIds] = React.useState<ConfigFileListFieldId[]>(selectedIds);

  const selectedFieldIds = React.useMemo(() => new Set(draftSelectedIds), [draftSelectedIds]);
  const visibleFields = React.useMemo(() => {
    return CONFIG_FILE_LIST_FIELDS.filter((field) => matchesSearch(field, search)).filter(
  ...
  const totalCount = CONFIG_FILE_LIST_FIELDS.length;
  ...
  const handleFieldToggle = function (fieldId: ConfigFileListFieldId, checked: boolean) {
  ...
  const handleVisibleFieldsToggle = function (checked: boolean) {
    setDraftSelectedIds((prev) => {
      if (checked) {
        return CONFIG_FILE_LIST_FIELDS.filter(
