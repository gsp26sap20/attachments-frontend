...
import { ATTACHMENT_LIST_FIELDS } from '@/features/attachments/view-config';
import type { AttachmentListFieldId, AttachmentListFieldOption } from '@/features/attachments/view-config';

function matchesSearch(field: AttachmentListFieldOption, search: string) {
...
interface AttachmentViewSettingsProps {
  onSelectChange?: (selectedIds: AttachmentListFieldId[]) => void;
}

export function AttachmentViewSettings({ onSelectChange }: AttachmentViewSettingsProps) {
  const selectedIds = useViewStore((state) => state.attachmentListVisibleFieldIds);
  const setSelectedIds = useViewStore((state) => state.setAttachmentListVisibleFieldIds);
  const [open, setOpen] = React.useState(false);
  const [search, setSearch] = React.useState('');
  const [showSelectedOnly, setShowSelectedOnly] = React.useState(false);
  const [draftSelectedIds, setDraftSelectedIds] = React.useState<AttachmentListFieldId[]>(selectedIds);

  const selectedFieldIds = React.useMemo(() => new Set(draftSelectedIds), [draftSelectedIds]);
  const visibleFields = React.useMemo(() => {
    return ATTACHMENT_LIST_FIELDS.filter((field) => matchesSearch(field, search)).filter(
  ...
  const totalCount = ATTACHMENT_LIST_FIELDS.length;
  ...
  const handleFieldToggle = function (fieldId: AttachmentListFieldId, checked: boolean) {
  ...
  const handleVisibleFieldsToggle = function (checked: boolean) {
    setDraftSelectedIds((prev) => {
      if (checked) {
        return ATTACHMENT_LIST_FIELDS.filter(
