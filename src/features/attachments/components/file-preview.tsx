...
export function FilePreview({
  ...
  if (preview.isPdf) {
    return (
      <div
        dangerouslySetInnerHTML={{ __html: preview.pdfHtml }}
  ...
  if (preview.isText) {
    return (
      <div
        className={cn(
          "w-auto h-auto min-h-60 border border-dashed p-4 rounded-lg",
          className,
        )}
      >
        <pre className="whitespace-pre-wrap wrap-break-word overflow-auto">
          {preview.textContent}
        </pre>
      </div>
    );
